...
import static latte.redis.clients.jedis3.params.SetParams.setParams;
import static latte.redis.clients.jedis3.tests.utils.AssertUtil.assertByteArrayListEquals;
...
public class BinaryValuesCommandsTest extends JedisCommandTestBase {
  ...
  public void sendCommandTest() {
    Object obj = jedis.sendCommand(Command.SET, "x".getBytes(), "1".getBytes());
    String returnValue = SafeEncoder.encode((byte[]) obj);
    assertEquals("OK", returnValue);
    obj = jedis.sendCommand(Command.GET, "x".getBytes());
    returnValue = SafeEncoder.encode((byte[]) obj);
    assertEquals("1", returnValue);

    jedis.sendCommand(Command.RPUSH, "foo".getBytes(), "a".getBytes());
    jedis.sendCommand(Command.RPUSH, "foo".getBytes(), "b".getBytes());
    jedis.sendCommand(Command.RPUSH, "foo".getBytes(), "c".getBytes());

    obj = jedis.sendCommand(Command.LRANGE, "foo".getBytes(), "0".getBytes(), "2".getBytes());
  ...
  public void sendBlockingCommandTest() {
    assertNull(jedis.sendBlockingCommand(Command.BLPOP, bfoo, Protocol.toByteArray(1L)));

    jedis.sendCommand(Command.RPUSH, bfoo, bbar);
    List<byte[]> blpop = (List<byte[]>) jedis.sendBlockingCommand(Command.BLPOP, bfoo,
