...
public class ClusterBinaryValuesCommandsTest extends ClusterJedisCommandsTestBase {
  ...
  public void testBinaryGeneralCommand() {
    ...
    jedisCluster.sendCommand("z".getBytes(), Command.SET, key, value);
    jedisCluster.sendCommand("y".getBytes(), Command.INCR, key);
    Object returnObj = jedisCluster.sendCommand("w".getBytes(), Command.GET, key);
    assertEquals("2", SafeEncoder.encode((byte[]) returnObj));
  }

  @Test
  public void testGeneralCommand() {
    jedisCluster.sendCommand("z", Command.SET, "x", "1");
    jedisCluster.sendCommand("y", Command.INCR, "x");
    Object returnObj = jedisCluster.sendCommand("w", Command.GET, "x");
    assertEquals("2", SafeEncoder.encode((byte[]) returnObj));
  }
