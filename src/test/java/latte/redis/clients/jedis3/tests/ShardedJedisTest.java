...
public class ShardedJedisTest {
  ...
  public void checkKeyTags() {
    ...
    Assert.assertEquals("foo", jedis.getKeyTag("foo"));
    Assert.assertEquals("bar", jedis.getKeyTag("foo{bar}"));
    Assert.assertEquals("bar", jedis.getKeyTag("foo{bar}}")); // Default pattern is non greedy
    Assert.assertEquals("bar", jedis.getKeyTag("{bar}foo")); // Key tag may appear anywhere
    Assert.assertEquals("bar", jedis.getKeyTag("f{bar}oo")); // Key tag may appear anywhere
    ...
    Assert.assertEquals("foo", jedis2.getKeyTag("foo"));
    Assert.assertNotEquals("bar", jedis2.getKeyTag("foo{bar}"));
  ...
  public void testGeneralCommand() {
    ...
    shards.add(si);
    ShardedJedis jedis = new ShardedJedis(shards);
    jedis.sendCommand(Command.SET, "a", "bar");
    JedisShardInfo s1 = jedis.getShardInfo("a");
    jedis.sendCommand(Command.SET, "b", "bar1");
    JedisShardInfo s2 = jedis.getShardInfo("b");
    jedis.disconnect();
    ...
    assertEquals("PONG", SafeEncoder.encode((byte[]) jedis.sendCommand(Command.PING)));
